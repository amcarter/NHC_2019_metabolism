...
dat <- dd %>% 
  filter((DateTime_EST > ymd_hms("2019-01-01 13:00:00") & 
          DateTime_EST < ymd_hms("2020-01-30 00:00:00"))) %>%
...
gas <- dat %>% 
  mutate(CO2.sat_umol = K0_CO2(0, dat$temp.water) * CO2_atm,
...
ghg <- read_csv("data/gas_data/compiled_ghg_dataframe_allvariables.csv")
ghg_nhc <- ghg %>% 
  mutate(del_CO2 = (CO2.ugL - CO2.sat) / 44, 
         del_O2 = (DO.obs - calc_DO_sat(watertemp_C,
                                        AirPres_kPa * 10))*1000 / 32) %>%
#  filter(site == "NHC") %>%
  select(del_CO2, del_O2, site, discharge_m3s, watertemp_C) %>%
  filter(site != "MC751")
...
png("figures/gas/CO2O2_grab_NHC_siteslopes.png", width = 6, height = 6,
    units = "in", res = 300)
Q <- ggplot(ghg_nhc, aes(del_CO2, del_O2, color = log(discharge_m3s))) +
  geom_point(size = 3) +
  geom_abline(slope = -1, intercept = 0) +
 scale_color_gradientn(colors = plasma(6)[1:5], name = "log(Q)") +
  xlab("excess CO2 (umol/L)") +
  ylab("excess O2 (umol/L)") +
