...
png(filename="figures/cumulative_pool_run_plot.png", width=450, height=450)
  ...
  plot(1,1, ylim = c(0, 28), xlim = c(1,366),xlab="",ylab="",xaxt="n",yaxt="n", bty="n", type="n")
  ...
  par(new=T)
  plot(cum_sites$DOY, cum_sites$GPP, xlim=c(0,366),ylim = c(-1400,300),type="n",
  ...
  text(x = c(as.numeric(format(t, "%j")),366), y = par("usr")[3]-45 , labels = format(t, "%b"), 
        pos = 1, xpd=NA, cex=1)
  axis(2, las=2, line=0, xpd=NA, tck=-.02, labels=FALSE,
  ...
  par(new=T)
  plot(metab_filled[metab_filled$site=="NHC",]$DOY, metab_filled[metab_filled$site=="NHC",]$discharge.m3s,
  ...
  text(x = -14, y = c(0.01, 1, 100) , labels = c("0.01", "1", "100"), 
       pos = 2, xpd=NA, cex=1)
  mtext(expression(paste("discharge m"^"3"~"/s")), side=2, line=2.5, adj=.05)
